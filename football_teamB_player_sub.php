<?php
    session_start();
    echo $_SESSION['football_teamB'].'<br>';
...
//echo $name."<br/>".$position;
  // $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    VALUES ('$name', '$position','$team','$matchno','0')";
    
    if ($conn->query($sql) === TRUE) {
        $q="select * from footballplayer where team='$team' AND matchno='$matchno' AND play='0'";
        $res=mysqli_query($conn,$q);
        $a= mysqli_num_rows($res);
        echo "Enter".($a+1). "player Name";
        if($a==4){
            header('location:adminLiveFootball.php');
        }
    } else {
    ...
    ?>
       <h1 style="text-align:center;"><span class="heading">Enter substitute player of<span style="font-size:3rem;text-transform:uppercase;color:red;font-weight:900"> <?php echo $_SESSION['football_teamB']?></span></span></h1>
